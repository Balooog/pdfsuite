...
2. Implement the job-runner service that streams CLI logs so every panel can remain thin wrappers over `pdfsuite`.
3. Wire at least one panel end-to-end (Pages or OCR) to validate UX before replicating the pattern across the remaining workflows.
